...
void parse_request(const char *clntAddr_str, const char *webRoot, int clntSock, int servSock){
...
static int handleFileRequest_fork(const char *clntAddr_str, const char *webRoot, int clntSock, int servSock)
{
	pid_t child_pid;
	child_pid = fork();
	...
	if(child_pid >= 0){
		if(child_pid == 0){  // Child1
			parse_request(clntAddr_str, webRoot, clntSock, servSock);
...
int main(int argc, char *argv[])
{
	...
	for (;;) {
		...
		if (clntSock < 0)
			die("accept() failed");
handleFileRequest_fork(inet_ntoa(clntAddr.sin_addr), webRoot, clntSock, servSock);
