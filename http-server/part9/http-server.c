...
void handleFileRequest_pthread(const char *webRoot, int servSockNum, int maxServSock){
	sigset_t set;
	sigemptyset(&set);
    ...
    for (;;) {
        ...
        FD_ZERO(&rset);
        // Initialize file descriptor list for select
        for (int i = 0; i < servSockNum; i++) {
            FD_SET(servSocks[i], &rset);
        }
        select(maxServSock + 1, &rset, NULL, NULL, NULL);
        
        for (i = 0; i < servSockNum; i++){
            if(FD_ISSET(servSocks[i], &rset)){
                clntSock = accept(servSocks[i], (struct sockaddr *)&clntAddr, &clntLen);
                if (clntSock < 0)
		{
		    if(errno==EWOULDBLOCK)
		    {
		         perror("no client to accept");
		    }
		    if(errno==EAGAIN)
		    {
		         perror("no connection, try again");
		    }
		    continue;
		}
...
int main(int argc, char *argv[])
{
    ...
    const char *webRoot = argv[argc - 1] ;
    int servSockNum;
    int maxServSock;
    ...
    for (int i = 1; i < argc - 1; i++) {
        ...
        if(maxServSock < servSocks[i - 1]){
            maxServSock = servSocks[i - 1];
        }
    }
	...
	//for (;;) {
		...
		// initialize the in-out parameter
				handleFileRequest_pthread(webRoot, servSockNum, maxServSock);
